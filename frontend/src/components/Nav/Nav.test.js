...
test('Contains hardcoded text', () => {
  ...
  expect(elem.textContent).toContain("Profile");
  expect(elem.textContent).toContain("Climbs");
});

/*

test('FontAwesome icon import is working', () => {
  const elem = render(
    <Router>
      <Nav />
    </Router>
  ...
  expect(elem.textContent).toContain("currentColor");
  expect(elem.className).toEqual('Pokemon');
});
*/